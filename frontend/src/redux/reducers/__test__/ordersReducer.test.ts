...
describe('ordersReducer', () => {
  ...
  it('should handle addStockInOrder', () => {
    ...
    expect(ordersReducer(undefined, action)).toEqual({
      ...initialState,
      orders: [stockOrder],
    });
  });
  ...
  it('should handle removeStockInOrder', () => {
    const state = {
      ...initialState,
      orders: [stockOrder, { ...stockOrder, selected: true }],
    };
  ...
  it('should handle selectStockInOrder', () => {
    ...
    expect(ordersReducer(state, action)).toEqual({
      ...state,
      orders: [{ ...stockOrder, selected: true }],
    });
  });
  ...
  it('should handle unselectStockInOrder', () => {
    ...
    expect(ordersReducer(state, action)).toEqual({
      ...state,
      orders: [{ ...stockOrder, selected: false }],
    });
  });
  ...
  it('should handle updateStockOrderExecutionMode', () => {
    ...
    expect(ordersReducer(state, action)).toEqual({
      ...state,
      orders: [{ ...stockOrder, executionMode }],
    });
  });
  ...
  it('should handle placeStockOrder', () => {
    ...
    expect(ordersReducer(state, action)).toEqual({
      ...initialState,
      orders: [{ ...s, status: 'In Progress', selected: false }],
    });
  });
  ...
  it('should handle placeStockOrderSuccess', () => {
    ...
    expect(ordersReducer(state, action)).toEqual({
      ...initialState,
      orders: [{ ...stockOrder, status: 'Booked', orderPrice }],
    });
  });
  ...
  it('should handle acknowledgeStockOrderError', () => {
    const state = {
      ...initialState,
      orders: [
        { ...stockOrder, error: 'test', status: 'Rejected' as OrderStatus },
      ],
    };
    const action = acknowledgeStockOrderError(0);
    expect(ordersReducer(state, action)).toEqual({
      ...initialState,
      orders: [{ ...stockOrder, status: 'Rejected', error: null }],
    });
  });
});
